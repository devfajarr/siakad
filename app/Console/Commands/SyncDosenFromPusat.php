    ...
    public function handle()
    {
        ...
        try {
            ...
            foreach ($dosenList as $dosenData) {
                try {
                    // Fetch detail biodata for extra fields (tempat_lahir, etc.)
                    // NOTE: The DetailBiodataDosen API is unreliable — it may return
                    // incorrect data (wrong dosen). We only use it for fields NOT present
                    // in the list API response, and SKIP it if the data looks suspicious.
                    try {
        ...
        foreach ($penugasanList as $penugasan) {
            DosenPenugasan::updateOrCreate(
                ...
                [
                    ...
                    'id_prodi' => $penugasan['id_prodi'], // pastikan referensi prodi ada atau biarkan string UUID
