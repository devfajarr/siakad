...
class SarprasController extends Controller
{
    ...
    {
        Log::info("SYNC_PULL: Mengakses daftar Manajemen Sarpras", ['endpoint' => route('admin.sarpras.index')]);

        $sarpras = Sarpras::with(['dosen', 'pegawai'])->latest()->get();
        $dosens = Dosen::orderBy('nama_lengkap')->get();
        $pegawais = Pegawai::orderBy('nama_lengkap')->get();

        return view('admin.jabatan.sarpras.index', compact('sarpras', 'dosens', 'pegawais'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'id_dosen' => 'required_if:tipe_user,dosen|nullable|exists:dosens,id|unique:sarpras,id_dosen',
            'id_pegawai' => 'required_if:tipe_user,pegawai|nullable|exists:pegawais,id|unique:sarpras,id_pegawai',
        ], [
        ...
        try {
            ...
            $model = Sarpras::create($data);

            Log::info("CRUD_CREATE: User dicalonkan menjadi Jabatan Sarpras", ['id' => $model->id, 'data' => $data]);
    ...
    public function update(Request $request, Sarpras $sarpra)
    {
        ...
        try {
            $sarpra->update(['is_active' => $request->is_active]);
            Log::info("CRUD_UPDATE: Status Jabatan ID {$sarpra->id} diubah", ['id' => $sarpra->id, 'changes' => $request->all()]);
    ...
    public function destroy(Sarpras $sarpra)
    {
        try {
            $sarpra->delete();
            Log::warning("CRUD_DELETE: Jabatan dihapus/soft-delete", ['id' => $sarpra->id]);
