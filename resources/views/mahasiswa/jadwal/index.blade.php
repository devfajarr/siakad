        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="mb-0">Jadwal Perkuliahan Saya</h5>
                        <small class="text-muted">Semester: {{ $activeSemester->nama_semester ?? $id_semester }}</small>
                    </div>
                    <form action="{{ route('mahasiswa.jadwal.index') }}" method="GET" class="d-flex align-items-center">
                                ...
                                <div class="tab-pane fade {{ $num == $todayNum ? 'show active' : '' }}" id="navs-{{ $num }}"
                                    ...
                                    @if (empty($jadwalGrouped[$num]))
                                        <div class="text-center py-5">
                                            ...
                                            <p class="text-muted">Tidak ada jadwal perkuliahan pada hari {{ $name }}.</p>
                                        </div>
                                    @else
                                        <div class="row g-4">
                                                    ...
                                                    <div class="card h-100 border shadow-none {{ $num == $todayNum ? 'border-primary' : '' }}">
                                                        <div class="card-body p-3">
                                                            ...
                                                            <div class="mt-2 pt-2 border-top">
                                                                <small class="text-muted d-block mb-1 text-uppercase fw-semibold" style="font-size: 0.65rem;">Dosen Pengajar:</small>
                                                                <p class="mb-0 text-dark small fw-medium">{{ $item['dosen'] }}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
